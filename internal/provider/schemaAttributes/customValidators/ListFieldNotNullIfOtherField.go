...
var _ validator.Object = &listFieldNotNullIfOtherFieldValidator{}

type listFieldNotNullIfOtherFieldValidator struct {
	targetField  path.Expression
	fieldToCheck path.Expression
	checkValue   string
}

func (s listFieldNotNullIfOtherFieldValidator) ValidateObject(ctx context.Context, request validator.ObjectRequest, response *validator.ObjectResponse) {
...
func (s listFieldNotNullIfOtherFieldValidator) Description(_ context.Context) string {
	return fmt.Sprintf("The field '%s' must not be null if the field '%s' is set to '%s'", s.targetField, s.fieldToCheck, s.checkValue)
}

func (s listFieldNotNullIfOtherFieldValidator) MarkdownDescription(ctx context.Context) string {
	return s.Description(ctx)
}

func ListFieldNotNullIfOtherField(targetField path.Expression, fieldToCheck path.Expression, checkValue string) validator.Object {
	return &listFieldNotNullIfOtherFieldValidator{
